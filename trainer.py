...
def main_worker(gpu, nprocs, args):
    ...
    args.model_save_path = os.path.join(
        args.save_folder, f"{args.model['encoder']}/{args.criterion['name']}/model")

    args.result_save_path = os.path.join(
        args.save_folder, f"{args.model['encoder']}/{args.criterion['name']}/result")
    ...
    for epoch in range(int(init_epoch), int(args.number_of_epochs + 1)):
        ...
        if args.gpu == 0:
            print('+-' * 8, time.strftime("%Y-%m-%d %H:%M:%S"), epoch,
                  "[INFO] Training %s with LR %f" % (args.model['encoder'], max(clr)), '-+' * 8)
...
def train(args):
    ...
    with open(os.path.join(args.save_folder, f"{args.model['encoder']}/{args.criterion['name']}/result/settings.txt"), 'a+') as settings_file:
        settings_file.write(
